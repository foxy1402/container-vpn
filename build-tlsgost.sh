...
MAX_RETRIES=3
RETRY_DELAY=5
IMAGE_NAME="tlsgost-proxy"
DOCKERFILE="Dockerfile.tlsgost"
...
main() {
    log "Starting TLSGost proxy build process (version: $VERSION)"
    ...
    log "Run with: docker run -d -p 127.0.0.1:8443:8443 -e TLSGOST_USER=<user> -e TLSGOST_PASS=<strong-random-password> ${IMAGE_NAME}:${VERSION}"
}
